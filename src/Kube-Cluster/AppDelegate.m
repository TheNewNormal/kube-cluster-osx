...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    NSString *home_folder = [NSHomeDirectory() stringByAppendingPathComponent:@"kube-cluster"];
    
    BOOL isDir;
    if([[NSFileManager defaultManager] fileExistsAtPath:home_folder isDirectory:&isDir] && isDir)
    // if kube-cluster folder exists
    {
        // set resouces_path
        ...
        [[NSFileManager defaultManager] createFileAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"kube-cluster/.env/resouces_path"]
        ...
        [[NSFileManager defaultManager] createFileAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"kube-cluster/.env/version"]
    ...
    else
    {
        ...
        [alert setMessageText:@"Kube-Cluster was not set."];
        ...
        {
            // Cancel clicked
            NSString *msg = [NSString stringWithFormat:@"%@ ", @" 'Initial setup of Kube-Cluster' at any time later one !!! "];
            [self displayWithMessage:@"You can set Kube-Cluster from menu 'Setup':" infoText:msg];
        }
    }
}
...
- (IBAction)update_k8s:(id)sender {
    ...
    else
    {
        // send a notification on to the screen
        NSUserNotification *notification = [[NSUserNotification alloc] init];
        notification.title = @"Kube-Cluster and";
...
- (IBAction)initialInstall:(id)sender
{
    ...
    if([[NSFileManager defaultManager]
        ...
        [self displayWithMessage:@"Kube-Cluster" infoText:msg];
    }
    else
    {
        ...
        NSString *scriptName = [[NSString alloc] init];
        NSString *arguments = [[NSString alloc] init];
        [self runScript:scriptName = @"kube-cluster-install" arguments:arguments = _resoucesPathFromApp ];
    }
}
// Setup menu
...
//    NSString *app_version = [NSString stringWithFormat:@"%@%@.%@", @"v", version, build];
    ...
    NSString *mText = [NSString stringWithFormat:@"%@ %@", @"Kube-Cluster for OS X", app_version];
    NSString *infoText = @"It is a simple wrapper around xhyve + CoreOS VM, which allows to control Kube-Cluster via Status Bar !!!";
...
- (IBAction)fleetUI:(id)sender {
    ...
    {
        NSLog (@"VM is On");
        NSString *file_path = [NSHomeDirectory() stringByAppendingPathComponent:@"kube-cluster/.env/ip_address"];
        // read IP from file
...
- (IBAction)quit:(id)sender {
    ...
    else
    {
        ...
        // send a notification on to the screen
        NSUserNotification *notification = [[NSUserNotification alloc] init];
        notification.title = @"Kube-Cluster";
        notification.informativeText = @"VM will be stopped";
        [[NSUserNotificationCenter defaultUserNotificationCenter] deliverNotification:notification];
    ...
    // send a notification on to the screen
    NSUserNotification *notification = [[NSUserNotification alloc] init];
    notification.title = @"Quitting Kube-Cluster App";
    [[NSUserNotificationCenter defaultUserNotificationCenter] deliverNotification:notification];
