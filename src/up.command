...
rm -f ~/kube-cluster/.env/password 2>&1 >/dev/null
...
if [ ! -f $HOME/kube-cluster/root.img ]; then
    echo " "
...
spin='-\|/'
i=1
while [ ! -f ~/kube-cluster/.env/ip_address ]; do i=$(( (i+1) %4 )); printf "\r${spin:$i:1}"; sleep .1; done
# get VM IP
